...
protected:
  ...
  uint8_t n;      // Quantidade de bits restantes para leitura no byte
...
int main(int argc, char *argv[])
{
  ...
  uint8_t bit;
  while ((bit = bl.le_bit()) != 2) // diferente de 2 pois esta na função que esse é o codigo de erro quando nao da pra ler mais
    printf("Leu bit %u\n", bit);

  FILE *f = fopen(argv[2], "wb");
  BufferBitsLeitura be(f);
...
uint8_t BufferBitsLeitura::le_bit()
{
  ...
  fread(byte, sizeof(uint8_t), 1, arquivo);
